 ...
 * Define Constants
 ****************************************/
const char *UBIDOTS_TOKEN = "BBFF-lXuUt3ig1cuX1AumSHj7xuhusXIjH8";  //Token relacionado con el dispositivo Ubidots
...
const char *VARIABLE_LABEL = "statusfoco"; // Variable de la cual extraeremos la infomración para encender o apagar el ventilador
...
void callback(char *topic, byte *payload, unsigned int length)//función que extrae la información de la variable para verificar si el valor ha cambiado o no
{
  ...
  if(value==0)
  {
    digitalWrite(relevador, LOW);//si el valor es cero entonces apaga el relevador
    delay(20);
  }else{
    digitalWrite(relevador, HIGH);//si el valor es diferente de cero entonces enciende el relevador
    delay(20);
  }
